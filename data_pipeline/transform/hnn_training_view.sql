CREATE OR REPLACE VIEW cr3bp.hnn_training_view AS
SELECT
    ...
    ts.ay,
    ts.az
FROM cr3bp.cr3bp_trajectory_sample AS ts
JOIN cr3bp.cr3bp_simulation_run AS run
    ON run.run_id = ts.run_id
JOIN cr3bp.cr3bp_system AS sys
    ON sys.system_id = run.system_id
JOIN cr3bp.cr3bp_lagrange_point AS lp
