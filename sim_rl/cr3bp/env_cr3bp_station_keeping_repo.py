"""
Gymnasium-compatible CR3BP station-keeping environment (Repo Version).

This version implements deterministic observation scaling and specific
reward weights for reproducible reference trajectory tracking.
"""
...
class Cr3bpStationKeepingEnvRepo(gym.Env):
    """
    Station-keeping environment in the normalized CR3BP (Deterministic Repo Version).

    This environment applies hard-coded scaling to observations to ensure
    numerical stability and reproducibility without relying on rolling-average
    normalization wrappers.

    Frame
    -----
    Rotating synodic frame with angular velocity omega = 1.

    Observation
    -----------
    Concatenated vector [scaled_dpos, scaled_dvel].
    Raw values are multiplied by SCALE_POS and SCALE_VEL respectively.
    ...
    def __init__(
        ...
        # Use Repo specific weights for reference tracking
        if self.use_reference_orbit:
            self.w_pos = float(W_POS_REPO)
            self.w_vel = float(W_VEL_REPO)
            self.w_ctrl = float(W_CTRL_REPO)
            self.w_planar = float(W_PLANAR_REPO)
        else:
            # Fallback to standard weights if reference orbit is disabled
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        # We scale raw values to a range suitable for the neural network (approx [-3, 3])
        # using fixed constants instead of a running average wrapper.
    ...
    def step(self, action):
        ...
        # Calculate Rewards based on physical distance (unscaled)
        rel_pos = sat.position - self.target
        rel_vel = sat.velocity - self.vel_ref_current
