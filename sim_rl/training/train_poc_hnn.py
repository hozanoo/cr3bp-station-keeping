...
This script mirrors the structure of the Repo/Robust training scripts
(logging, callbacks, reproducibility) but utilizes the Physics-Informed
Neural Network (HNN) for dynamics propagation.
"""
...
# ======================================================================
# 1. HARD DETERMINISM ENFORCEMENT
# ======================================================================
...
N_ENVS = 4
BASE_RUN_DIR = Path(__file__).resolve().parent / "runs_hnn"
...
def make_run_dirs(scenario: ScenarioConfig) -> dict[str, Path]:
    """
    Create directory structure: runs_hnn/<scenario_name>/run_<timestamp>
    """
...
def save_hnn_run_config(config_dir: Path, scenario: ScenarioConfig, model: PPO):
    """
    Save HNN-specific configuration.
    """
    cfg = {
...
def rollout_policy_to_csv(
    ...
    Uses the HNN environment logic.
    """
    # Create a fresh environment for recording
    env = Cr3bpStationKeepingEnvHNN(scenario=scenario, use_reference_orbit=use_reference_orbit)
...
def make_env_fn(scenario: ScenarioConfig, rank: int, use_reference_orbit: bool, log_dir: Path):
    """
    Create a deterministic HNN environment instance.
    Includes Monitor for CSV logging.
    """
    def _thunk():
        env_seed = GLOBAL_SEED + rank
        
        env = Cr3bpStationKeepingEnvHNN(
            scenario=scenario,
