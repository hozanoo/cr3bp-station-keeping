"""
Training script for the HNN-accelerated CR3BP environment (Robust Version).

This script enforces strict reproducibility and uses the Robust HNN environment,
which combines HNN dynamics with domain randomization and external disturbances.
"""
...
from sim_rl.cr3bp_hnn.env_cr3bp_station_keeping_hnn_robust import Cr3bpStationKeepingEnvHNNRobust
...
N_ENVS = 4
BASE_RUN_DIR = Path(__file__).resolve().parent / "runs_hnn_robust"
...
def save_hnn_robust_config(config_dir: Path, scenario: ScenarioConfig, model: PPO):
    cfg = {
        "type": "HNN_ROBUST_ACCELERATED",
        ...
        "robust_rewards": {
            "W_POS_ROBUST": W_POS_ROBUST,
            "W_VEL_ROBUST": W_VEL_ROBUST,
            "W_CTRL_ROBUST": W_CTRL_ROBUST,
            "W_PLANAR_ROBUST": W_PLANAR_ROBUST,
        },
        "ppo_hyperparams": {
...
def rollout_policy_to_csv(
    ...
    env = Cr3bpStationKeepingEnvHNNRobust(scenario=scenario, use_reference_orbit=use_reference_orbit)
...
class TqdmProgressCallback(BaseCallback):
    ...
    def _on_training_start(self) -> None:
        self.pbar = tqdm(total=self.total_timesteps, desc="Training (HNN Robust)", unit="step")
...
def make_env_fn(scenario: ScenarioConfig, rank: int, use_reference_orbit: bool, log_dir: Path):
    def _thunk():
        ...
        env = Cr3bpStationKeepingEnvHNNRobust(
            scenario=scenario,
...
def train_hnn_robust_agent(
    ...
    print(f"[INFO] Starting Robust HNN-Accelerated Training for: {scenario.name}")
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix="ckpt_hnn_robust"
    )
    
    eval_env = Cr3bpStationKeepingEnvHNNRobust(
        scenario=scenario, 
        use_reference_orbit=use_reference_orbit,
    ...
    final_path = dirs["models"] / "final_model_hnn_robust.zip"
    model.save(str(final_path))
    print(f"[INFO] Training finished. Model saved to {final_path}")
    
    save_hnn_robust_config(dirs["config"], scenario, model)

    export_eval_to_csv(dirs["logs"], dirs["logs"] / "eval_rewards.csv")


if __name__ == "__main__":
    train_hnn_robust_agent(
