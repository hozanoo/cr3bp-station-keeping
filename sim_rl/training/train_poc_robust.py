"""
Determinist training script for the CR3BP station-keeping task (Robust Version).
...
3. Using the deterministic 'Robust' environment.
"""
...
N_ENVS = 4
BASE_RUN_DIR = Path(__file__).resolve().parent / "runs_robust"
...
def make_run_dirs(scenario: ScenarioConfig) -> dict[str, Path]:
    """
    Create directory structure: runs_robust/<scenario_name>/run_<timestamp>
...
def save_robust_config(config_dir: Path, scenario: ScenarioConfig, model: PPO):
    """
    Save the configuration including the specific Robust constants.
    """
    cfg = {
...
def rollout_policy_to_csv(
    ...
    Uses the Robust environment logic.
    """
    # Create a fresh environment for recording
    env = Cr3bpStationKeepingEnvRobust(scenario=scenario, use_reference_orbit=use_reference_orbit)
...
class TqdmProgressCallback(BaseCallback):
    ...
    def _on_training_start(self) -> None:
        self.pbar = tqdm(total=self.total_timesteps, desc="Training (Robust)", unit="step")
...
def train_robust(
    ...
    print(f"[INFO] Starting ROBUST Training for: {scenario.name}")
